...
// tagType 表示 NBT 标签的类型。
//
// tagType represents the type of NBT tag.
type tagType byte

// String 将 tagType 转换为字符串形式，格式类似 `TAG_` 加标签类型，例如 `TAG_Byte`。
//
...
// IsValid 检查 tagType 是否为有效且已知的标签类型。
//
...
// tagFromType 将 reflect.Type 匹配到能够承载该值的 NBT 标签类型；找不到匹配时返回 math.MaxUint8。
//
