...
// offsetReader 包装 io.Reader，用于跟踪已读取数据的偏移量，方便错误携带位置信息。
//
...
// Read 从 io.Reader 读取数据，并按实际读取的字节数增加偏移。
//
